...
def index():
    with database_manager as session:
        ...
        for room in rooms:
            room.date_filtered = room.creation_date.strftime('%b %d %Y %I:%M.%S %p')
    ...
    return render_template('admin.html', rooms=rooms, logs=logs[::-1], server_files=server_files, data=data)
